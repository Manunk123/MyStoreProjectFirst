...
import com.mystore.pages.DeliverPage;
		 ...
		 public class ActionClass extends BaseClass  {
		 	...
		 	public void click2(WebDriver driver, By locator) {
		 	    ...
		 	    try {
		 	        ...
		 	        System.out.println("⚠ Stale element, re-locating...");
		 	        WebElement ele = driver.findElement(locator);
		 	        ele.click();
		 	    } catch (Exception e) {
		 	        System.out.println("⚠ Click failed, using Actions as last resort");
		 	        WebElement ele = driver.findElement(locator);
		 	        Actions act = new Actions(driver);
			...
			public boolean findElement(WebDriver driver, WebElement ele) {
		 	...
		 	/*public boolean moveToElement(WebDriver driver,) {
		 		boolean flag = false;
		 		try {
		 			// WebElement element = driver.findElement(locator);
		 			JavascriptExecutor executor = (JavascriptExecutor) driver;
		 ...
		 */
		 	...
		 	public boolean handleAlertIfPresent(WebDriver driver) {
		 	    ...
		 	    try {
		 	        ...
		 	        if (alertHandled) {
		 	            System.out.println("The alert was present and handled successfully.");
		 	        } else {
			...
			public void implicitWait(WebDriver driver, int timeOut) {
		 	...
		 	public String screenShot(WebDriver driver, String filename) {
		 	    ...
		 	    return destination; // Return full absolute path!
		 	}

		 		/*
		 		// This new path for jenkins
		 		String newImageString = "/ScreenShots/" + filename+"_"+ dateName + ".png";
		 	...
		 	public void selectOptionByText(WebElement dropdown, List<WebElement> options, String value) {
		 	    Actions action = new Actions(getDriver());
		 	    action.moveToElement(dropdown).click().build().perform(); // Open dropdown
		 	 ...
		 	 public void pressTab() {
		         try {
		             ...
		             System.out.println("❌ TAB key press failed: " + e.getMessage());
		         }
		     }
		     ...
		     public void pressTabOnElement(WebElement element) {
		         try {
		             new Actions(getDriver()).click(element).sendKeys(Keys.TAB).build().perform();
		             System.out.println("✅ TAB key pressed on element: " + element);
		         } catch (Exception e) {
		             System.out.println("❌ TAB key press on element failed: " + e.getMessage());
		         }
		     }
		     ...
		     public void pressBackspace() {
		         try {
		             ...
		             System.out.println("❌ BACKSPACE key press failed: " + e.getMessage());
		         }
		     }
		     ...
		     public void pressShiftTab() {
		         try {
		             new Actions(getDriver()).keyDown(Keys.SHIFT).sendKeys(Keys.TAB).keyUp(Keys.SHIFT).build().perform();
		             System.out.println("✅ SHIFT + TAB pressed");
		         } catch (Exception e) {
		             System.out.println("❌ SHIFT + TAB failed: " + e.getMessage());
		         }
		     }
		     ...
		     public void setQuantity(WebDriver driver, WebElement plusbutton, String qty) {
		    	    try {
		    	        // ✅ First, try direct sendKeys
		    	        plusbutton.click();
		    	        plusbutton.clear();
		    	        plusbutton.sendKeys(String.valueOf(qty));
		    	        System.out.println("✅ Quantity set via sendKeys: " + qty);
		    	    } catch (Exception e) {
		    	        System.out.println("⚠️ sendKeys failed, trying JavaScript...");
		    	        try {
		    	            // ✅ Fallback: use JavaScript
		    	            JavascriptExecutor js = (JavascriptExecutor) driver;
		    	            js.executeScript("arguments[0].value='" + qty + "'; arguments[0].dispatchEvent(new Event('change'));", plusbutton);
		    	            System.out.println("✅ Quantity set via JavaScript: " + qty);
		    	        } catch (Exception ex) {
		    	            System.out.println("❌ Failed to set quantity: " + ex.getMessage());
		    	        }
		    	    }
		    	}
		     
		     
		     public void increaseQuantity(WebDriver driver, By plusBtn, int count) {
		    	    WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));

		    	    for (int i = 0; i <= count; i++) {
		    	        try {
		    	            // ✅ Re-locate button every time
